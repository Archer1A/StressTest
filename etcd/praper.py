...
with open(os.devnull, 'w') as devnull:
    ...
    if openssl == 0:
        ...
        openssl = subprocess.Popen(
            ['openssl', 'req', '-new', '-x509', '-key', '/dev/stdin', '-days', '3650', '-subj', empty_subj],
        ...
        print(
            'Warning: auth_pkey and auth_cert cannot be generated automatically without openssl.  Please set it manually')
...
generate_template(os.path.join(template_dir, 'jobservice.cm.yaml'),
                  os.path.join(output_dir, 'jobservice/jobservice.cm.yaml'))
...
generate_template(os.path.join(template_dir, 'adminserver.cm.yaml'),
                  os.path.join(output_dir, 'adminserver/adminserver.cm.yaml'))
